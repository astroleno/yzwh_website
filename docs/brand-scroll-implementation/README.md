...
- 现有 `docs/assets` 是业务参考素材，不直接作为最终全屏视觉使用；当前 Hero 桌面主视频已放到 `/Users/aitoshuu/Documents/GitHub/yzwh_website/public/video/hero.webm`，并保留 `/video/hero.mp4` 作为 fallback。最终站点运行资产统一放到 repo 根目录的 `public/` 下。
...
3. 最后补齐和优化运行资产：Hero 视频已到位，桌面优先 4K H.264 MP4，移动端走轻量视频；继续生成风险立面、航线背景、报告缩略图、案例图层，并压缩上线版本。
